...
describe 'munin::client' do
  ...
  describe 'the munin-node.conf' do
    ...
    it 'is based on the template' do
      expect(chef_run).to
      create_template(conf_path).with(source: 'munin-node.conf.erb')
    end

    it 'is mode 0644' do
      expect(chef_run).to
      create_template(conf_path).with(mode: '0644')
    end
      ...
      context 'with a list of server names' do
        ...
        before do
          fake_dns.each_pair do |host, ip|
            allow(IPSocket).to
            receive(:getaddress).with(host).and_return(ip)
          end
        end
        ...
        it 'always allows access to localhost' do
          expect(chef_run).to
          render_file(conf_path).with_content('allow ^127\.0\.0\.1$')
        end

        it 'allows access to the servers' do
          fake_dns.values.each do |ip|
            expect(chef_run).to
            render_file(conf_path).with_content("allow ^#{ip.gsub('.', '\\.')}$")
          end
        end
