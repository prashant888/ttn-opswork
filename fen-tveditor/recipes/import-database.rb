...
bash 's3cmd-get' do
  ...
  user 'root'
  code <<-EOH
  /usr/bin/s3cmd get --force s3://fe-tv-database-backups/*.dump.gz /tmp/db_dumps
  EOH
  end
...
%w(biobase cdoyle kdebisschop webadmin www).each do |db_user|
...
db_list = %w(biobase events movies partner recalls rss status syndb tvusers botr chemgloss feature id names poll regdb scores sudoku tv)
...
%w(biobase cdoyle kdebisschop www).each do |db_user|
  postgresql_database_user db_user do
    action :drop
  end
end